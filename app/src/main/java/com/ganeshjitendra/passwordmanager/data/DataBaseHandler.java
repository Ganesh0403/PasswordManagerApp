package com.ganeshjitendra.passwordmanager.data;
...
public class DataBaseHandler extends SQLiteOpenHelper {
    ...
    public int insertExcelData(String table, ContentValues values) {
        try {
            SQLiteDatabase db = this.getWritableDatabase();

            int y = (int) db.insert(Constants.TABLE_NAME, null, values);
            db.close();
//            Toast.makeText(context, "y="+y, Toast.LENGTH_SHORT).show();
            return y;
        }catch (Exception c){
            Toast.makeText(context, "Error Insert"+ c.getMessage().toString(), Toast.LENGTH_SHORT).show();
            return 0;
        }
    }
    ...
    public List<Item> getAllItemsForExcel(){
        SQLiteDatabase db = this.getReadableDatabase();
...
//                item.setDateItemAdded(formattedDate);
                item.setDateItemAdded(cursor.getString(cursor.getColumnIndex(Constants.KEY_DATE_NAME)));
